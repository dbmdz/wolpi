...
public class ImageRequestParser {
    ...
    public Rotation parseRotation(String rotationSpec) {
        ...
        if (!iiifConfig.features().rotation().arbitrary() && degrees % 90.0 != 0) {
            throw new IllegalArgumentException("Arbitrary rotation is not supported in this configuration.");
        }
    ...
    public CropRectangle parseRegion(String cropSpec, ImageSize sourceSize) {
        ...
        if ("square".equalsIgnoreCase(cropSpec)) {
            ...
            if (!supported.square()) {
                throw new IllegalArgumentException("Square cropping is not supported in this configuration.");
            }
        ...
        if (cropSpec.toLowerCase().startsWith("pct:")) {
            if (!supported.byPercent()) {
                throw new IllegalArgumentException("Percent-based cropping is not supported in this configuration.");
            }
    ...
    public ImageSize parseSize(IIIFVersion iiifVersion, String sizeSpec, ImageSize sourceSize)
        ...
        if ("full".equals(sizeSpec) || "max".equals(sizeSpec) || "^max".equals(sizeSpec)) {
            if (limits.maxWidth() > 0) {
                ...
                if (result.width() > limits.maxWidth() || (doUpscale && result.width() < limits.maxWidth())) {
            ...
            if (limits.maxHeight() > 0) {
                ...
                if (result.height() > limits.maxHeight() || (doUpscale && result.height() < limits.maxHeight())) {
                    result = new ImageSize((int) Math.max(1, Math.floor(result.width() * scale)), (int)
                            Math.max(1, Math.floor(result.height() * scale)));
                }
            }
            ...
            if (!supported.byWidth()) {
                throw new IllegalArgumentException("Width-based scaling is not supported in this configuration.");
            }
            ...
            if (!supported.byHeight()) {
                throw new IllegalArgumentException("Height-based scaling is not supported in this configuration.");
            }
            ...
            if (parts.length != 2) {
                throw new IllegalArgumentException("Invalid confined width and height specification: " + sizeSpec);
            }
        ...
        boolean wasUpscaled = (result.width() > sourceSize.width() || result.height() > sourceSize.height());
        if (wasUpscaled) {
        ...
        if (limits.maxWidth() > 0 && result.width() > limits.maxWidth()) {
            throw new IllegalArgumentException("Requested width exceeds maximum allowed width: " + limits.maxWidth());
        }
        ...
        if (limits.maxArea() > 0 && (long) result.width() * result.height() > limits.maxArea()) {
            throw new IllegalArgumentException("Requested area exceeds maximum allowed area: " + limits.maxArea());
        }
        return result;
    }
    ...
    public @Nullable ImageRequest toCanonicalForm(ImageRequest request, ImageSize sourceSize) {
        ...
        try {
            ...
            boolean sizeIsMax = parsedSize.width() == iiifConfig.limits().maxWidth()
