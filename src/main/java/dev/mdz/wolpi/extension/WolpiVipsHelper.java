...
public class WolpiVipsHelper {
    ...
    public static VipsBandFormat image_get_band_format(VImage img) {
        return mapToEnum(VipsHelper.image_get_format(img.getUnsafeStructAddress()), VipsBandFormat.class);
    }

    public static VipsInterpretation image_get_interpretation(VImage img) {
        return mapToEnum(VipsHelper.image_get_interpretation(img.getUnsafeStructAddress()), VipsInterpretation.class);
    }

    private static <T extends VEnum> T mapToEnum(int rawVal, Class<T> enumClass) {
        return Arrays.stream(enumClass.getEnumConstants())
                .filter(v -> v.getRawValue() == rawVal)
                .findFirst()
